...
import { API, NKUHELPER_ROOT } from './flows_api';

import { load_config, save_config } from './Config';

const IMAGE_BASE = '//39.105.113.112/Tree_Hole_Login/uploadimage/';
const AUDIO_BASE = NKUHELPER_ROOT + 'services/pkuhole/audios/';
...
class FlowItem extends PureComponent {
  constructor(props) {
    ...
    this.storageLocation = this.storageLocation.bind(this);
    // this.componentWillUnmount =this.componentWillUnmount.bind(this);
  }
  componentDidMount() {
    this.props.change_imgShow;
  ...
  storageLocation() {
    // history.pushState(null, null, 'image.html');
    // location.hash = 'image';
    var imagesmall = document.getElementById('imageSmall');
    var url = imagesmall.src;
    var image = document.getElementById('imgPreview');
    var imagesrc = image.setAttribute('src', url);
    var imagePreview = document.getElementById('Preview');
    this.props.change_imgShow;
    imagePreview.style.display = 'block';
  }
  previewimageBack(event) {
    event.preventDefault();
      ...
      <div>
        ...
        >
          <img
            src=""
            id="imgPreview"
            onClick={this.previewimageBack.bind(this)}
          />
        </p>
        <div
          className={'flow-item' + (props.is_quote ? ' flow-item-quote' : '')}
        >
          {!!props.is_quote && (
          ...
          <div className="box">
            ...
            <div className="box-header">
              ...
              {!!parseInt(props.info.likenum, 10) && (
                <span className="box-header-badge">
                  ...
                  <span
                    className={
                      'icon icon-' + (this.props.attention ? 'star-ok' : 'star')
                    }
                  />
                </span>
              )}
            ...
            <div className="box-content">
              ...
              {props.info.type === 'image' && (
                <p className="img">
                  {props.img_clickable ? (
                    ...
                    //   target="_parent"
                    //   onClick={this.storageLocation.bind(this)}
                    // >
                    <img
              ...
              {props.info.type === 'audio' && (
                <AudioWidget src={IMAGE_BASE + props.info.url} />
              )}
            </div>
      ...
      <div className="flow-item-row sidebar-flow-item">
        <div className="box box-tip sidebar-toolbar">
          ...
          <span className="sidebar-toolbar-item">
            <Button
              variant="contained"
              onClick={this.load_replies.bind(this)}
              size="small"
            >
              <AutorenewIcon />
              刷新
          ...
          <span className="sidebar-toolbar-item">
            <Button
              variant="contained"
              onClick={this.toggle_rev.bind(this)}
              size="small"
            >
              <MobiledataOffIcon />
              逆序
...
class FlowItemRow extends PureComponent {
  ...
  show_sidebar() {
    //浏览器添加新的历史记录，便于监听用户的回退动作
    // location.hash = 'sidebar';
    history.pushState(null, null, 'sidebar.html');
    this.props.show_sidebar(
            ...
            >
              <div>
                ...
                <FlowItemRow
                  ...
                  change_imgShow={props.change_imgShow}
                />
              </div>
            </LazyLoad>
          ))}
    ...
    if (page === this.state.loaded_pages + 1) {
      ...
      if (this.state.mode === 'list') {
        ...
        API.get_attention(this.props.token)
          ...
          .catch(failed);
      } else if (this.state.mode === 'mine') {
        API.get_mine(this.props.token).then((json) => {
          this.setState((prev, props) => ({
            chunks: {
              title: 'Mine List',
              data: prev.chunks.data.concat(
          ...
          text={
            this.state.loading_status === 'loading' ? (
              ...
              <a href="http://beian.miit.gov.cn/">
                <font color="white">津ICP备2021007774号-1</font>
              </a>
            )
          }
        />
